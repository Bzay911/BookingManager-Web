    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-6">
        {stats.map(({ label, value, sub, icon: Icon, textClass, bgClass }) => (
          <Card
            key={label}
            className="border border-gray-100 shadow-sm rounded-3xl bg-white hover:shadow-md transition-shadow"
          >
            <CardContent className="p-6">
              ...
              <p className="text-3xl font-extrabold text-black mb-1 tracking-tight">
                {value}
              </p>
                ...
                return (
                  <div key={i} className="flex items-center gap-4 py-2 group">
                    ...
                    <div className="flex flex-col items-end gap-1.5">
                      <span
                        className={`text-xs font-bold px-2.5 py-1 rounded-full ${s.bgClass} ${s.textClass}`}
                      >
                        {s.label}
                      </span>
        ...
        <Card className="border border-gray-100 shadow-sm rounded-3xl bg-white">
          ...
          <CardContent className="px-8 pb-8 pt-6 space-y-3">
            {[
              ...
              >
                <div
                  className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${bg} group-hover:scale-105 transition-transform`}
                >
                ...
                <ChevronRight
                  size={16}
                  className="ml-auto text-gray-300 group-hover:text-[#0be48d] transition-colors"
                />
              </button>
            ))}
          </CardContent>
